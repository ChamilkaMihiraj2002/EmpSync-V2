...
const SuperAdmin = () => {
  ...
  const handleOk = async (values) => {
    if (!token) return;
    try {
      await axios.post(`${urL}/super-admin/organizations`, values, {
        headers: {
  ...
  const updateOrganization = async (orgId, values) => {
    if (!token) return;
    try {
      await axios.put(`${urL}/super-admin/organizations/${orgId}`, values, {
        headers: {
          Authorization: `Bearer ${token}`,
          'Content-Type': 'application/json',
        },
  ...
  const handleDelete = (item) => {
    confirm({
      ...
      async onOk() {
        if (!token) return;
        try {
          await axios.delete(`${urL}/super-admin/organizations/${item.key}`, {
            headers: { Authorization: `Bearer ${token}` },
          });
          await fetchOrganizations();
        } catch (err) {
